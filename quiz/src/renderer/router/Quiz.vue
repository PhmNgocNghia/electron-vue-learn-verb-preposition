    ...
    <div v-else-if="quizzes">
      ...
      <div
        ...
        <div class= "title mt-3">
          Câu {{index + 1}} / {{numWordLearnEachTime}} :
          <b v-if = "$store.state.settings.learnWhat === learnWhat.phrasalVerb">
            {{quiz.phrasalVerb.verb}}
          </b>
          <b v-else>
            {{createVprepOQuestion(quiz.phrasalVerb)}}
          </b>
        </div>
        <v-radio-group
...
export default {
  computed: {
    ...
    learnWhat () {
      return this.$store.state.settings.learnWhat
    }
  },

  methods: {
    createVprepOQuestion (pharasalVerb) {
      let question = pharasalVerb.verbOnly + ' '
